...
for atr in [0.4, 0.5]:  # 统一攻击者比例
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    LOG_FILE = f"log/lfighter_ae_{config.LOG_DIST_TYPE}_source{config.SOURCE_CLASS}_target{config.TARGET_CLASS}_atr{atr}_{timestamp}.log"
